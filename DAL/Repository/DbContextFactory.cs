...
namespace DAL
{
    public class DbContextFactory : IDbContextFactory, IDisposable
    {
        ...
        {
            var options = new DbContextOptionsBuilder<ArchiveDbContext>().UseNpgsql(settings.Value.DbConnectionString).EnableSensitiveDataLogging().Options;
                //,
                //npgsqlOptionsAction: s =>
                //{
                //  s.EnableRetryOnFailure(maxRetryCount: 10, maxRetryDelay: TimeSpan.FromSeconds(30), errorCodesToAdd: null);
                //}).Options;
            DbContext = new ArchiveDbContext(options);
        ...
        public ArchiveDbContext DbContext { get; private set; }
        /// <summary>
        /// Release DB context
        /// </summary>
        public void Dispose()
